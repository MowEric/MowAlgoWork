...
public class QuickHull {
    ...
    public QuickHull(List<Point2D> points) {
        ...
        Line2D line = new Line2D(points.get(0), points.get(points.size()-1));
        List<Point2D> left = leftSet(points, line);
        List<Point2D> temp = points;
        List<Point2D> right = points;
        ...
        points = temp;
        this.hull = findHull(left, line);
        line = line.reversed();
        this.hull.addAll(findHull(right,line));

        // right = points.removeAll(left);
